...
local KEY_STRING = "GARDENER_QUEST_KEY_1" .. getServerID()
local GARDENER_QUEST = GARDENER_QUEST or {}
local DAILY_LIMIT = 5
local SECONDS_IN_DAY = 86400

--[[Modular stuff here, editing these will reflect on the whole system.]]
local gemPenalty = 500
local completeIcon = 6292
local incompleteIcon = 25338
local gardenerId = 25148
...
local function saveQuestData()
    saveDataToServer(KEY_STRING, GARDENER_QUEST)
...
local quests = {
    {
        id = "harvest_trees",
        description =
        "`oCan you Harvest `w{harvest_seeds_goal} `otrees for me? They can be from any kind. I don't need the drops from the trees, I just need them harvested.",
        objectives = {
            { type = "harvest_seeds", goalRange = { 50, 250 } },
        },
        prizes = {
            gems  = { min = 250, max = 500 },
            items = {
                { id = 228,  amountRange = { 5, 20 } }, -- Spray Fertilizer
                { id = 5706,  amountRange = { 10, 50 } }, -- SSP
                { id = 25142, amountRange = { 1, 1 } }    -- FarmToken for every quest
            }
        }
    },
    {
        id = "harvest_rarity",
        description =
        "`oCan you Harvest `w{harvest_rarity_goal} `oRarity worth of trees for me? They can be from any kind. I don't need the drops from the trees, I just need them harvested.",
        objectives = {
            { type = "harvest_rarity", goalRange = { 1000, 10000 } },
        },
        prizes = {
            gems  = { min = 250, max = 500 },
            items = {
    ...
    },
    {
        id = "break_blocks",
        description =
        "`oCan you Break `w{break_blocks_goal} `oBlocks for me? They can be from any kind. I don't need the drops from the blocks, I just need them broken.",
        objectives = {
            { type = "break_blocks", goalRange = { 100, 1000 } },
        },
        prizes = {
            gems  = { min = 500, max = 1000 },
            items = {
                { id = 1778,  amountRange = { 1, 5 } }, -- Deluxe Spray 
                { id = 20230,  amountRange = { 5, 10 } }, -- RSP
                { id = 25142, amountRange = { 1, 2 } }    -- FarmToken for every quest
            }
        }
    },
    {
        id = "break_rarity",
        description =
        "`oCan you Break `w{break_rarity_goal} `oRarity worth of Blocks for me? They can be from any kind. I don't need the drops from the blocks, I just need them broken.",
        objectives = {
            { type = "break_rarity", goalRange = { 1000, 10000 } },
        },
        prizes = {
            gems  = { min = 500, max = 1000 },
            items = {
    ...
    },
    {
        id = "earnxp",
        description =
        "`oCan you earn `w{earnxp_goal} `oXP for me? I don't care how you earn the XP, just do it.",
        objectives = {
            { type = "earnxp", goalRange = { 1000, 10000 } },
        },
        prizes = {
            gems  = { min = 750, max = 1500 },
            items = {
                { id = 1488,  amountRange = { 1, 5 } }, -- XP Potion
                { id = 25142, amountRange = { 1, 3 } }    -- FarmToken for every quest
            }
        }
    }
}
...
local PROGRESS_FORMATS = {
    ...
    place_blocks = function(p, g)
        return string.format("`oPlace `w%s `oBlocks. (%s/%s)", formatNumberWithCommas(g),
            formatNumberWithCommas(p), formatNumberWithCommas(g))
    end,
    ...
    break_blocks = function(p, g)
        return string.format("`oBreak `w%s `oBlocks. (%s/%s)", formatNumberWithCommas(g),
            formatNumberWithCommas(p), formatNumberWithCommas(g))
    end,
    ...
    harvest_seeds = function(p, g)
        return string.format("`oHarvest `w%s `oTrees. (%s/%s)", formatNumberWithCommas(g),
            formatNumberWithCommas(p), formatNumberWithCommas(g))
    end,
...
local function buildGardenerGetDialog(player)
    local userId = player:getUserID()
    local record = GARDENER_QUEST[userId] or {}
    ...
    return
        "set_default_color|`o\n" ..
        "add_label_with_icon|big|`wGardener``|left|" .. gardenerId .. "|\n" ..
        "add_spacer|small|\n" ..
        "add_textbox|`oWelcome, looking for some quests to boost your progression? I could use an extra hand. If you wish to embark on a random quest, just say so!``|left|\n" ..
        "add_spacer|small|\n" ..
        "add_button|garderContinueQuest|`9Give me a Quest!|noflags|0|0|\n" ..
        "add_smalltext|`oQuests Completed: " .. formatNumberWithCommas(completedCount) .. "``|left|\n" ..
        text ..
        "end_dialog|gardenerGetQuest|No Thanks||"
end

--[[This will show up when the player clicks on 'continue' when they don't have an active quest after wrenching the NPC.]]
local function buildGardenerDisclaimerDialog()
    return
        "set_default_color|`o\n" ..
        "add_label_with_icon|big|`wGardener``|left|" .. gardenerId .. "|\n" ..
        "add_spacer|small|\n" ..
        "add_smalltext|`oJust to let you know, you may turn in your quests at any Gardener you have access to (we're in a union).``|left|\n" ..
        "add_spacer|small|\n" ..
        ...
        "add_spacer|small|\n" ..
        "add_button|gardenerStartQuest|`9Alright, Give me the Quest!|noflags|0|0|\n" ..
        "end_dialog|gardenerDisclaimerDialog|No thanks||"
end

--[[Actual quest dialog, it shows the quest's description, player's progress, etc.]]
local function buildGardenerActiveQuestDialog(questData)
    local dialog =
        "set_default_color|`o\n" ..
        "add_label_with_icon|big|`wGardener's Quest``|left|" .. gardenerId .. "|\n" ..
        "add_spacer|small|\n" ..
    ...
    if allComplete then
        dialog = dialog .. "add_button|gardenerCompleteQuest|`oComplete Quest``|noflags|0|0|\n"
    else
        dialog = dialog .. "add_button|gardenerNotComplete|`oI will be back!``|noflags|0|0|\n"
    end

    dialog = dialog .. "add_button|gardenerCancelQuest|`oGive up``|noflags|0|0|\n"
    dialog = dialog .. "end_dialog|gardenerActiveQuestDialog|Goodbye!||"
    return dialog
end

--[[This will show up when the player tries to cancel the quest. Pretty much like a confirmation page.]]
local function buildGardenerEndQuestDialog()
    return
        "set_default_color|`o\n" ..
        ...
        "add_spacer|small|\n" ..
        "add_button|gardenerEndQuest|`wGive up this Quest|noflags|0|0|\n" ..
        "end_dialog|gardenerEndQuestDialog|Nevermind||"
end

--[[This handles if a player wrenches on a wrenchable block.]]
onTileWrenchCallback(function(world, player, tile)
    --[[Replace the value of gardenerId to the wrenchable block/NPC you want.
    DM me if you want this to handle multiple different NPCs.]]
    if tile:getTileID() ~= gardenerId then return false end

    local userId = player:getUserID()
    local record = GARDENER_QUEST[userId]
    local questData = record and record.currentQuest

    if questData then
        player:onDialogRequest(buildGardenerActiveQuestDialog(questData))
    else
        player:onDialogRequest(buildGardenerGetDialog(player))
    end
    return true
end)
...
onPlayerDialogCallback(function(world, player, data)
    ...
    if dialog ~= "gardenerGetQuest"
        and dialog ~= "gardenerActiveQuestDialog"
        and dialog ~= "gardenerDisclaimerDialog"
        and dialog ~= "gardenerEndQuestDialog"
    then
        return false
    end

    local playerData = GARDENER_QUEST[userId] or {}
    local completedCount = playerData.completedCount or 0
    local quest = playerData.currentQuest

    --[[If the player clicks on 'Continue']]
    if button == "garderContinueQuest" then
        player:onDialogRequest(buildGardenerDisclaimerDialog())
        --[[if the player clicks on 'Yes!']]
    elseif button == "gardenerStartQuest" then
    ...
    if record.dailyCount >= DAILY_LIMIT then
        ...
        saveQuestData()
        player:onDialogRequest(buildGardenerActiveQuestDialog(GARDENER_QUEST[userId].currentQuest))
        --[[If the player clicks on 'Give up this quest']]
    elseif button == "gardenerCancelQuest" then
        player:onDialogRequest(buildGardenerEndQuestDialog())
        --[[If the player clicks on 'Yes!']]
    elseif button == "gardenerEndQuest" then
    ...
    elseif button == "gardenerCompleteQuest" then
        local record = GARDENER_QUEST[userId]
        if not record then
    ...
    elseif button == "gardenerNotComplete" then
...
local function addQuestProgress(player, type, amount)
    ...
    for _, milestone in ipairs(milestones) do
        if prevPercent < milestone and newPercent >= milestone then
            player:onConsoleMessage(string.format("`9Gardener's Task: %s is `2%d%% complete!``", taskName, milestone))
        end
    end

    if oldProgress < obj.goal and obj.progress >= obj.goal then
        player:onConsoleMessage(string.format("`9Gardener's Task: %s is `2completed!``", taskName))
    end
    ...
    if allComplete and not quest.notified then
        ...
        player:onConsoleMessage("`9Quest Complete! Go talk to the Gardener.``")
        player:onTalkBubble(player:getNetID(), "`9Quest Complete! Go talk to the Gardener.``", 0)
        quest.notified = true
        saveQuestData()
    end
end
...
registerLuaCommand({
    command = "resetgardenerquestdata",
    roleRequired = 51,
    description = "Resets GARDENER_QUEST data."
})
onPlayerCommandCallback(function(world, player, fullCommand)
    local command = fullCommand:match("^(%S+)$")
    local userId = player:getUserID()

if command == "resetgardenerquestdata" then
    if not player:hasRole(51) then
        player:onConsoleMessage("`4Error: `wYou need Developer role to use this command!``")
        return true
    end
    if next(GARDENER_QUEST) == nil then
        player:onConsoleMessage("`oData: `wGARDENER_QUEST `ois already empty! Nothing to reset here.``")
    else
        GARDENER_QUEST = {}
        saveQuestData()
        player:onConsoleMessage("`oData: `wGARDENER_QUEST `ohas been reset.``")
    end
...
loadQuestsData()
for userId, record in pairs(GARDENER_QUEST) do
