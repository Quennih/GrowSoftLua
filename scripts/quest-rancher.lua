...
local KEY_STRING = "RANCHER_QUEST_KEY_" .. getServerID()
local RANCHER_QUEST = RANCHER_QUEST or {}
local DAILY_LIMIT = 5
...
local function saveQuestData()
    saveDataToServer(KEY_STRING, RANCHER_QUEST)
...
local quests = {
    {
        id = "harvest_provider",
        description =
        "`oCan you Harvest `w{harvest_provider_goal} `oProviders for me? Cow, chicken, science station. If It provides, It counts! You can keep what you get from the providers.",
        objectives = {
            { type = "harvest_provider", goalRange = { 50, 250 } },
        },
        prizes = {
            gems  = { min = 2500, max = 4000 },
            items = {
                { id = 228,  amountRange = { 5, 20 } }, -- Spray Fertilizer
                { id = 5706,  amountRange = { 10, 50 } }, -- SSP
                { id = 25140, amountRange = { 1, 1 } }    -- FarmToken for every quest
            }
        }
    },
    {
        id = "harvest_tackle",
        description =
        "`oCan you Harvest `w{harvest_tackle_goal} `oProviders for me? If It provides, It counts! You can keep what you get from the providers.",
        objectives = {
            { type = "harvest_tackle", goalRange = { 50, 250 } },
        },
        prizes = {
            gems  = { min = 2500, max = 4000 },
            items = {
...
local PROGRESS_FORMATS = {
    ...
    harvest_provider = function(p, g)
        return string.format("`oHarvest `w%s `oProviders. (%s/%s)", formatNumberWithCommas(g),
            formatNumberWithCommas(p), formatNumberWithCommas(g))
    end
}
...
local TASK_FORMATS = {
    ...
    harvest_provider = function(goal) return "Harvest " .. formatNumberWithCommas(goal) .. " Providers" end
}
...
local function buildRancherGetDialog(player)
    local userId = player:getUserID()
    local record = RANCHER_QUEST[userId] or {}
    ...
    return
        "set_default_color|`o\n" ..
        "add_label_with_icon|big|`wRancher``|left|" .. rancherId .. "|\n" ..
        "add_spacer|small|\n" ..
        ...
        "add_spacer|small|\n" ..
        "add_button|rancherContinueQuest|`9Give me a Quest!|noflags|0|0|\n" ..
        "add_smalltext|`oQuests Completed: " .. formatNumberWithCommas(completedCount) .. "``|left|\n" ..
        text ..
        "end_dialog|rancherGetQuest|No Thanks||"
end


--[[This will show up when the player clicks on 'continue' when they don't have an active quest after wrenching the NPC.]]
local function buildRancherDisclaimerDialog()
    return
        "set_default_color|`o\n" ..
        "add_label_with_icon|big|`wRancher``|left|" .. rancherId .. "|\n" ..
        "add_spacer|small|\n" ..
        "add_smalltext|`oJust to let you know, you may turn in your quests at any Rancher you have access to (we're in a union).``|left|\n" ..
        "add_spacer|small|\n" ..
        ...
        "add_spacer|small|\n" ..
        "add_button|rancherStartQuest|`9Alright, Give me the Quest!|noflags|0|0|\n" ..
        "end_dialog|rancherDisclaimerDialog|No thanks||"
end

--[[Actual quest dialog, it shows the quest's description, player's progress, etc.]]
local function buildrancherActiveQuestDialog(questData)
    local dialog =
        "set_default_color|`o\n" ..
        "add_label_with_icon|big|`wRancher's Quest``|left|" .. rancherId .. "|\n" ..
        "add_spacer|small|\n" ..
    ...
    if allComplete then
        dialog = dialog .. "add_button|rancherCompleteQuest|`oComplete Quest``|noflags|0|0|\n"
    else
        dialog = dialog .. "add_button|rancherNotComplete|`oI will be back!``|noflags|0|0|\n"
    end

    dialog = dialog .. "add_button|rancherCancelQuest|`oGive up``|noflags|0|0|\n"
    dialog = dialog .. "end_dialog|rancherActiveQuestDialog|Goodbye!||"
    return dialog
end

--[[This will show up when the player tries to cancel the quest. Pretty much like a confirmation page.]]
local function buildrancherEndQuestDialog()
    return
        "set_default_color|`o\n" ..
        ...
        "add_spacer|small|\n" ..
        "add_button|rancherEndQuest|`wGive up this Quest|noflags|0|0|\n" ..
        "end_dialog|rancherEndQuestDialog|Nevermind||"
end

--[[This handles if a player wrenches on a wrenchable block.]]
onTileWrenchCallback(function(world, player, tile)
    --[[Replace the value of rancherId to the wrenchable block/NPC you want.
    DM me if you want this to handle multiple different NPCs.]]
    if tile:getTileID() ~= rancherId then return false end

    local userId = player:getUserID()
    local record = RANCHER_QUEST[userId]
    local questData = record and record.currentQuest

    if questData then
        player:onDialogRequest(buildrancherActiveQuestDialog(questData))
    else
        player:onDialogRequest(buildRancherGetDialog(player))
    end
    return true
end)
...
onPlayerDialogCallback(function(world, player, data)
    ...
    if dialog ~= "rancherGetQuest"
        and dialog ~= "rancherActiveQuestDialog"
        and dialog ~= "rancherDisclaimerDialog"
        and dialog ~= "rancherEndQuestDialog"
    then
        return false
    end

    local playerData = RANCHER_QUEST[userId] or {}
    local completedCount = playerData.completedCount or 0
    local quest = playerData.currentQuest

    --[[If the player clicks on 'Continue']]
    if button == "rancherContinueQuest" then
        player:onDialogRequest(buildRancherDisclaimerDialog())
        --[[if the player clicks on 'Yes!']]
    elseif button == "rancherStartQuest" then
    ...
    if record.dailyCount >= DAILY_LIMIT then
        ...
        saveQuestData()
        player:onDialogRequest(buildrancherActiveQuestDialog(RANCHER_QUEST[userId].currentQuest))
        --[[If the player clicks on 'Give up this quest']]
    elseif button == "rancherCancelQuest" then
        player:onDialogRequest(buildrancherEndQuestDialog())
        --[[If the player clicks on 'Yes!']]
    elseif button == "rancherEndQuest" then
    ...
    elseif button == "rancherCompleteQuest" then
        local record = RANCHER_QUEST[userId]
        if not record then
    ...
    elseif button == "rancherNotComplete" then
...
local function addQuestProgress(player, type, amount)
    ...
    if allComplete and not quest.notified then
        ...
        player:onConsoleMessage("`9Quest Complete! Go talk to the Rancher.``")
        player:onTalkBubble(player:getNetID(), "`9Quest Complete! Go talk to the Rancher.``", 0)
        quest.notified = true
        saveQuestData()
    end
end
...
onPlayerProviderCallback(function(world, player, tile, itemID, itemCount)
    ...
    if quest.objectives["harvest_provider"] then
        addQuestProgress(player, "harvest_provider", 1)
    end
    if quest.objectives["harvest_tackle"] and tileId == tackleboxId then
...
registerLuaCommand({
    command = "resetrancherquestdata",
    roleRequired = 51,
    description = "Resets RANCHER_QUEST data."
})

onPlayerCommandCallback(function(world, player, fullCommand)
    local command = fullCommand:match("^(%S+)$")
    if command ~= "resetrancherquestdata" then return false end
    if not player:hasRole(51) then return false end

    if next(RANCHER_QUEST) == nil then
        player:onConsoleMessage("`oData: `wRANCHER_QUEST `ois already empty! Nothing to reset here.``")
    else
        RANCHER_QUEST = {}
        saveQuestData()
        player:onConsoleMessage("`oData: `wRANCHER_QUEST `ohas been reset.``")
    end
...
loadQuestsData()
for userId, record in pairs(RANCHER_QUEST) do
